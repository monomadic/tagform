...
| `d` | fetch the page's tags with `yt-dlp`, from anywhere on the form (metadata only, no download) and stage them onto Title, Actors, Channel, Description, Tags and Date — a field the page does not answer keeps its value; `u` takes the whole fetch back |
