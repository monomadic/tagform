...
pub static PALETTES: &[Palette] = &[
    ...
    // look emitted rather than printed, so they run hot -- and where the
    ...
    // 16 colours as the accents. The hardware palette itself is too dark against
    ...
    // The amber CRT, whose whole gamut was one phosphor. The accents stay
    // inside it: warm hues only, separated by lightness and by how far each
    // sits toward red or green, because a cool colour here would read as a
    // bug in the monitor rather than as a colour choice.
    Palette {
        name: "amber",
